...
export default function UserProvider({children}){
    ...
    const [deliveryguy, setDeliveryguy]=useState(false)
 ...
 async function fetchData  () {
     ...
     try {
      const response = await axiosConfig.get("/user/user", {
       ...
       if (response.status === 200 && response.data.RegisterAs === "Admin") {
         setAdmin(true);
         setUser(response.data);
         setIsLoggedin(true);
         navi('/admin')
         console.log(token)
       }else if(response.status === 200 && response.data.RegisterAs === "DeliveryGuy") {
          setAdmin(false);
          setDeliveryguy(true)
          setIsLoggedin(true);
